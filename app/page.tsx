...
export default function Home(){
 ...
 const [reviewers,setReviewers]=useState(['AI Architect (Zain Israr)','System Architect']); const [tags,setTags]=useState(['Task Management','Operations','GSolve Pilot']); const [tag,setTag]=useState(''); const [file,setFile]=useState(''); const [selectedFile,setSelectedFile]=useState<File | null>(null); const [selectedProject,setSelectedProject]=useState<Project>(); const [message,setMessage]=useState(''); const [activeStep,setActiveStep]=useState(0); const [activeNav,setActiveNav]=useState(0); const input=useRef<HTMLInputElement>(null);
 ...
 const submit=(kind:string)=>handleSubmit(async (data)=>{
   ...
   const fields: Record<string,string>={...data, project_type:selectedProject.project_type, target_end_date:selectedProject.target_end_date, business_domain:selectedProject.business_domain, sub_domain:selectedProject.sub_domain, priority:selectedProject.priority, budget:selectedProject.budget, project_manager:selectedProject.manager, project_priority:selectedProject.priority, start_date:selectedProject.start_date, assign_to:data.assignee, client:selectedProject.customer, regulatory_impact:'Low', reviewers:reviewers.join(', '), currency:selectedProject.currency_name, project_name:selectedProject.project_name, last_updated:selectedProject.last_updated, status:selectedProject.project_status, project_code:selectedProject.project_code, target_release_date:'30/09/2026', project_stage:'Planning', tags:tags.join(', '), updated_by:selectedProject.updated_by, contract_reference:selectedProject.contract_reference, complexity:'Medium'};
