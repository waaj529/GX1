...
import type { Project } from './lib/gsolve';
...
const schema = z.object({ title:z.string().min(1,'BRD title is required'), description:z.string().min(1,'Short description is required'), notes:z.string(), assignee:z.string().min(1), business_domain:z.string(), sub_domain:z.string(), complexity:z.string(), intake_project_type:z.string(), regulatory_impact:z.string(), priority:z.string(), target_release_date:z.string() });
...
export default function Home(){
 const {register,handleSubmit,watch,formState:{errors}}=useForm<FormData>({defaultValues:{title:'',description:'',notes:'',assignee:'',business_domain:'',sub_domain:'',complexity:'',intake_project_type:'',regulatory_impact:'',priority:'',target_release_date:''}});
 const [reviewers,setReviewers]=useState<string[]>([]); const [tags,setTags]=useState<string[]>([]); const [tag,setTag]=useState(''); const [file,setFile]=useState(''); const [selectedFile,setSelectedFile]=useState<File | null>(null); const [selectedProject,setSelectedProject]=useState<Project>(); const [message,setMessage]=useState(''); const [activeStep,setActiveStep]=useState(0); const [activeNav,setActiveNav]=useState(0); const input=useRef<HTMLInputElement>(null);
 ...
 const choose=(f?:File)=>{if(!f)return;if(f.size>50*1024*1024||!/(pdf|vnd.openxmlformats-officedocument.wordprocessingml.document)$/.test(f.type))setMessage('Please choose a PDF or DOCX file under 50 MB.');else {setSelectedFile(f);setFile(f.name);setMessage('Document attached successfully.')}};
 const submit=(kind:string)=>handleSubmit(async (data)=>{
   const check=schema.safeParse(data); if(!check.success){setMessage('Please complete all required fields.');return;}
   if(kind==='Draft'){setMessage('Draft saved locally.');return;}
   if(!selectedProject){setMessage('Select a GSolve project first.');return;}
   if(!selectedFile){setMessage('Please attach the BRD PDF or DOCX file first.');return;}
   const form=new FormData();
   const fields: Record<string,string>={...data, project_type:data.intake_project_type || selectedProject.project_type, target_end_date:selectedProject.target_end_date, business_domain:data.business_domain || selectedProject.business_domain, sub_domain:data.sub_domain || selectedProject.sub_domain, priority:data.priority || selectedProject.priority, budget:selectedProject.budget, project_manager:selectedProject.manager, project_priority:data.priority || selectedProject.priority, start_date:selectedProject.start_date, assign_to:data.assignee, client:selectedProject.customer, regulatory_impact:data.regulatory_impact, reviewers:reviewers.join(', '), currency:selectedProject.currency_name, project_name:selectedProject.project_name, last_updated:selectedProject.last_updated, status:selectedProject.project_status, project_code:selectedProject.project_code, target_release_date:data.target_release_date, project_stage:'', tags:tags.join(', '), updated_by:selectedProject.updated_by, contract_reference:selectedProject.contract_reference, complexity:data.complexity};
   Object.entries(fields).forEach(([key,value])=>form.append(key,String(value ?? ''))); form.append('brd_file',selectedFile); setMessage('Submitting BRD for processing…');
   try { const response=await fetch('/api/brd',{method:'POST',body:form}); const payload=await response.json(); if(!response.ok)throw new Error(payload.message||'BRD submission failed.'); setMessage(`${payload.message||'BRD submitted successfully.'}${payload.data_id?` Data ID: ${payload.data_id}`:''}`); } catch(error){setMessage(error instanceof Error?error.message:'BRD submission failed.');}
 })();
 ...
 return <div className={`shell screen-${activeStep + 1} ${activeStep === 3 ? 'screen-spec-shell' : ''}`}>
  <aside className="sidebar"><div className="brand"><button className="hamb" aria-label="Open navigation"><I n="menu"/></button><img className="brand-logo" src="/brand/gx1-logo.png" alt="GX1 Platform — AI-Driven Intelligence"/></div><div className="nav-scroll"><div className="nav" style={{ marginBottom: 12 }}><NavIcon name="intake"/><span className="nav-label">Dashboard</span></div><p className="group">BRD PIPELINE <ChevronDown size={14}/></p>{nav.map(([i,name],x)=><div key={name} role="button" tabIndex={0} onClick={()=>{
    setActiveNav(x);
  ...
  <main className="app"><header><div><h1>GX1 – BRD to Production Platform</h1><p>{getHeaderSubtitle()}</p></div><div className="head-right"><span className="bell"><I n="bell"/><b>12</b></span><I n="help"/><div className="avatar large">RS</div><div><strong>Rohit Sharma</strong><small>Business Analyst</small></div><span>⌄</span><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', marginLeft: '4px' }}><span className="gx-logo-badge" style={{ margin: 0 }}>GREEN</span><span style={{ fontSize: '8px', color: '#078541', fontStyle: 'italic', fontWeight: 600 }}>Future: Envisioned</span></div></div></header>
   ...
   {activeStep===0&&<div className="content"><div className="workspace"><div className="title"><h2>BRD Intake &amp; Registration</h2><p>Capture and register a new Business Requirement Document (BRD) against a GSolve project.</p></div><div className="notice"><I n="info"/> Project details and metadata are pulled from GSolve and cannot be edited here.</div>
    <ProjectSelection onMessage={setMessage} onProjectChange={setSelectedProject}/>
    <form onSubmit={e=>{e.preventDefault();submit('BRD')}} className="form-grid"><Section title="2. &nbsp; BRD INFORMATION"><Label required>BRD Title</Label><input {...register('title')} aria-invalid={!!errors.title}/><Label required>Short Description</Label><div className="counter-wrap"><textarea {...register('description')} maxLength={500}/><small>{watch('description').length} / 500</small></div></Section><Section title="3. &nbsp; DOCUMENT UPLOAD"><Label required>Upload BRD Document (PDF/DOCX)</Label><div className="drop" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();choose(e.dataTransfer.files[0])}} onClick={()=>input.current?.click()}><I n="upload"/><b>Drag &amp; drop file here</b><span>or click to browse</span><input ref={input} type="file" accept=".pdf,.docx" onChange={e=>choose(e.target.files?.[0])}/></div>{file&&<div className="file"><span className="pdf">PDF</span><div><b>{file}</b><small>2.4 MB</small></div><I n="check"/><button aria-label="Remove file" onClick={()=>setFile('')}><I n="trash"/></button></div>}<p className="upload-note">Max file size: 50 MB <i/> Accepted formats: PDF, DOCX</p></Section>
     <Section title="4. &nbsp; CLASSIFICATION"><div className="two"><div><Label>Business Domain</Label><select {...register('business_domain')}><option value="">Select business domain</option><option>Operations</option></select></div><div><Label>Sub Domain</Label><select {...register('sub_domain')}><option value="">Select sub domain</option><option>Task Management</option></select></div></div><div className="three"><div><Label required>Complexity</Label><select {...register('complexity')}><option value="">Select complexity</option><option>Medium</option></select></div><div><Label required>Project Type</Label><select {...register('intake_project_type')}><option value="">Select project type</option><option>Internal Development</option></select></div><div><Label required>Regulatory Impact</Label><select {...register('regulatory_impact')}><option value="">Select regulatory impact</option><option>Low</option></select></div></div></Section><Section title="5. &nbsp; ASSIGNMENT"><Label required>Assign To</Label><select {...register('assignee')}><option value="">Select assignee</option><option>UX-CX Lead (Jahir Hussain)</option></select><Label required>Reviewers</Label><div className="tag-select">{reviewers.map(r=><span key={r}>{r}<button type="button" onClick={()=>setReviewers(reviewers.filter(x=>x!==r))}>×</button></span>)}<b>⌄</b></div></Section>
     <Section title="6. &nbsp; PRIORITY &amp; TARGET"><div className="two"><div><Label>Priority</Label><select {...register('priority')}><option value="">Select priority</option><option>High</option><option>Medium</option><option>Low</option></select></div><div><Label>Target Release Date</Label><div className="date"><input {...register('target_release_date')} placeholder="DD/MM/YYYY"/><I n="cal"/></div></div></div></Section><Section title="7. &nbsp; TAGS &amp; NOTES <small>(Optional)</small>"><div className="two tag-notes"><div><Label>Tags</Label><div className="tag-input">{tags.map(t=><span key={t}>{t}<button type="button" onClick={()=>setTags(tags.filter(x=>x!==t))}>×</button></span>)}<input value={tag} onChange={e=>setTag(e.target.value)} onKeyDown={addTag} placeholder="Type and press Enter"/></div></div><div><Label>Notes</Label><div className="counter-wrap"><textarea {...register('notes')} maxLength={500} placeholder="Enter notes (optional)"/><small>{watch('notes').length} / 500</small></div></div></div></Section></form>
